    ...
    if cuda_available:
        try:
            ...
            _model = AutoModelForCausalLM.from_pretrained(
                MODEL_ID, quantization_config=bnb_cfg, device_map="auto", token=hf_token,
            )
            print("[llm] Loaded in 4-bit NF4 on GPU.")
...
def _extract_json(text: str) -> dict:
    ...
    if m:
        try:
            ...
            pass
    # Try to find last valid JSON object by scanning from the end
...
def _normalize_relevance(raw) -> float:
    try:
        v = float(raw)
...
def judge_goal_progress(
    ...
    """Judge goal progress after a session.
...
def generate(prompt: str, schema: dict, lang: str = "en") -> dict:
    """General-purpose structured generation. Appends schema hint and parses JSON output."""
    ...
    if not result:
        raise ValueError(
            f"Could not parse JSON from model output (len={len(raw)}):\n{raw[:600]}"
        )
    return result
